...
#' @param sppPA.fut.file character. OPTIONAL. File path to the table of species presences
...
#'
#' @export
compilePextSext <- function(masterBL.file, masterScen.file, sppPA.fut.file = NULL, redo = FALSE) {
  ...
  if (grepl("hd_rcp85",  masterScen.file)) {
    if (length(sppPA.fut.file)) {
      sppPA.fut <- as.data.table(get(load(sppPA.fut.file)))
    } else {
      stop("Please provide 'sppPA.fut.file'")
    }
