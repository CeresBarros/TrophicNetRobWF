...
for (params in parameters) {
  ...
  if (!exists("orig.pixhabs2")) {
    if (exists("orig.pixhabs")) {
      orig.pixhabs2 <- copy(orig.pixhabs)
      rm(orig.pixhabs)
    } else {
      stop("Can't find pix X habitat matrix")
    }
  }

  if (is(orig.pixhabs2, "data.table")) {
    orig.pixhabs2 <- as.data.frame(orig.pixhabs2)
    rownames(orig.pixhabs2) <- orig.pixhabs2$PAGENAME
  }


  if (!exists("BARM.habs2")) {
    if (exists("BARM.habs")) {
      BARM.habs2 <- BARM.habs
      rm(BARM.habs)
    } else {
      stop("Can't find species X habitat matrix")
    }
  }
