...
Require::Require(c(
  ...
  "ade4",
  "RSQLite",
  "PredictiveEcology/reproducible@b4664aa7b9e51e9135a3b5f408ccde2de2d44a21",
  "CeresBarros/ToolsCB"
), 
...
Require::Require("qs", libPaths = file.path("packages/"), require = FALSE)


options("future.globals.maxSize" = +Inf)
options("reproducible.useNewDigestAlgorithm" = 2)
options("reproducible.cachePath" = "cache/runs2021")


## requires
# .libPaths("/home/cbarros/Networks/R/4.0/")
.libPaths("packages/")
if (!"Require" %in% installed.packages())
  install.packages("Require", lib = "packages/")

Require::Require(c("PredictiveEcology/reproducible@b4664aa7b9e51e9135a3b5f408ccde2de2d44a21",
                   "CeresBarros/ToolsCB",
                   "factoextra"), 
                 libPaths = file.path("packages/")
                 # libPaths = "/home/cbarros/Networks/R/4.0/"
...
options(nwarnings = 100000000, max.print = 100000000)
...
options(repos =  c("predictiveecology.r-universe.dev", 
...
Require::Require(c(
  ...
  "ggpubr",
  "PredictiveEcology/reproducible@b4664aa7b9e51e9135a3b5f408ccde2de2d44a21",
  "CeresBarros/ToolsCB@development (HEAD)"
),
libPaths = file.path("packages/"))
...
source("Rscripts/Tools/compilePextSext.R")
source("Rscripts/Tools/summStatsByPixSpp.R")
source("Rscripts/Tools/plotFuns.R")
...
## 10K grid poly
mask10kSHP <- shapefile("Masks/10k/Shapefiles/grid_10km/grid_10Km.shp")
## 10K grid raster in final projection
mask10k <- raster("Masks/10k/reference_grid_10km.img")

mask10k[] <- mask10k[]  ## bring raster to memory

## lakes raster
## TODO: put data in Zenodo and download from there
lakes <- raster("data/Habitats/wise_large_lakes/lakes10k.img")
```

## Habitat data preparation

1.  Calculate the the area of each GlobCover (v2.2) habitat category
    (300m2) per pixel -- hereafter referred to as a "pixel" --
    of `mask10k`.
...
```{r}
## setting directories
output.dir <- "data/Habitats/Habitat_pixel/GlobCover"
dir.create(output.dir)

## habitats
habRasterFile <- "data/Habitats/GLOBCOVER.img"

source("Dataprep/HABITATdata.R")

objsToRm <- setdiff(ls(all.names = TRUE), 
                    c("mask10k", "mask10kID", "lakes"))  ## keep these 
rm(list = objsToRm)
...
1.  Make pixel X species matrices from projected species distributions
    (`SPPdata.R`). Species distributions were projected under baseline
    climate conditions (climate normals for 1960-1990) and future
    climate conditions for 2050 (average for 2041-2060). Future climate
    ...
    scenario (equivalent to CMIP6 SSP5-8.5 -- see above). All
    projections available in [Zenodo](URL). Species were allowed to
    follow their climate niche without restrictions.

2.  Get the IUCN status of each tetrapod species (`sppIUCNstatus.R`).
...
## file suffix to use throughout
file.suff <- ifelse(opt, "_opthab_tresh", "_allhab_tresh")

source("Dataprep/SPPdata.R")
...
## GET PARAMETERS --------------------------------------
## Baseline networks
parameters <- read.table("Param_files/2021/params_BL.txt", sep = ";")$V1
...
2.  Quantiles values calculated: 10%, 25%, 50%, 75% and 90%. Minimum and
